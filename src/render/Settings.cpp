...
Settings::Settings() {
	...
	m_settings["randomSeed"] = std::to_string(0xACE1u);
}
...
std::string& Settings::operator[](std::string first) {
	if (m_settings.find(first) == m_settings.end()) {
		m_settings[first] = "";
	}

	return m_settings[first];
}
