...
void DebugScene::Create(Settings& settings) {
	...
	m_renderedObjects.push_back(new Sphere(1, m_matMap["unshaded"], Vector3D::Zero, Vector3D(-5.25, 1, 0)));
	m_renderedObjects.push_back(new Sphere(1, m_matMap["dielectric"], Vector3D::Zero, Vector3D(-3.15, 1, 0)));
	m_renderedObjects.push_back(new Sphere(1, m_matMap["diffuse"], Vector3D::Zero, Vector3D(-1.05, 1, 0)));
